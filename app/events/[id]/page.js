...
import { ChevronLeft, ChevronRight, MapPin, Calendar, ShieldCheck, Loader2, User, Mail, CheckCircle2, Download, Printer } from 'lucide-react';
...
export default function EventPage() {
  ...
  const handleDownloadPDF = () => {
    window.print(); // This triggers the browser's "Save as PDF" dialog
  };
  ...
  const recordPayment = async (response, tier) => {
    ...
    if (error) {
      ...
      alert("Payment OK, but DB Error: " + error.message);
    } else {
      setPaymentSuccess({
        reference: response.reference,
        tier: tier.name,
    ...
    return (
      <div className="printable-ticket" style={{ maxWidth: '500px', margin: '40px auto', padding: '20px' }}>
        ...
        <div style={{ textAlign: 'center', padding: '40px', background: '#fff', borderRadius: '32px', boxShadow: '0 20px 50px rgba(0,0,0,0.1)', border: '1px solid #f1f5f9' }}>
          ...
          <h2 style={{ fontSize: '24px', fontWeight: 900 }}>TICKET CONFIRMED</h2>
          ...
          <div style={{ background: '#000', color: '#fff', padding: '20px', borderRadius: '24px', textAlign: 'left', marginBottom: '25px' }}>
             <p style={{ margin: '0 0 5px 0', fontSize: '12px', opacity: 0.7 }}>ATTENDEE</p>
             <p style={{ margin: '0 0 15px 0', fontWeight: 700 }}>{paymentSuccess.customer}</p>
             <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                <div>
                  <p style={{ margin: '0 0 5px 0', fontSize: '12px', opacity: 0.7 }}>TIER</p>
                  <p style={{ margin: 0, fontWeight: 700 }}>{paymentSuccess.tier}</p>
                </div>
                <div style={{ textAlign: 'right' }}>
                  <p style={{ margin: '0 0 5px 0', fontSize: '12px', opacity: 0.7 }}>PRICE</p>
          ...
          <div style={{ marginBottom: '20px' }}>
            ...
            <p style={{ marginTop: '10px', fontSize: '12px', fontWeight: 800, color: '#94a3b8' }}>REF: {paymentSuccess.reference}</p>
          </div>

          <div style={{ display: 'flex', gap: '10px' }} className="no-print">
            <button onClick={handleDownloadPDF} style={{ ...checkoutBtn(false), background: '#f1f5f9', color: '#000', flex: 1 }}>
              <Download size={18} style={{ marginRight: '8px' }} /> SAVE PDF
            </button>
            <button onClick={() => window.location.reload()} style={{ ...checkoutBtn(false), flex: 1 }}>
              DONE
            </button>
          </div>
        </div>
  ...
  return (
    <div style={{ maxWidth: '1000px', margin: '0 auto', padding: '40px 20px' }}>
      <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 1fr', gap: '40px' }}>
        <div style={{ position: 'relative', borderRadius: '40px', overflow: 'hidden', height: '600px', background: '#f1f5f9' }}>
        ...
        <div>
          ...
          <form onSubmit={handlePurchase}>
            <div style={{ marginBottom: '25px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
               <h3 style={{ fontWeight: 800, fontSize: '16px' }}>Attendee Details</h3>
            ...
            <h3 style={{ fontWeight: 800, fontSize: '16px', marginBottom: '12px' }}>Choose Ticket</h3>
...
const checkoutBtn = (dis) => ({ width: '100%', background: dis ? '#cbd5e1' : '#000', color: '#fff', padding: '20px', borderRadius: '20px', border: 'none', fontWeight: 900, fontSize: '16px', marginTop: '25px', cursor: dis ? 'not-allowed' : 'pointer', display: 'flex', justifyContent: 'center', alignItems: 'center' });
