...
export default function CreateEvent() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (invalidTier) {
      setFormError("Please ensure all tiers have a name, price, and valid quantity (min 1).");
      return;
    }
  ...
  // --- 6. STYLING ---
  const styles = {
    pageContainer: { padding: '24px 16px 100px', maxWidth: '1280px', margin: '0 auto', minHeight: '100vh', backgroundColor: '#fcfdfe', fontFamily: '"Inter", sans-serif', boxSizing: 'border-box', overflowX: 'hidden' },
    topHeader: { display: 'flex', flexDirection: 'column', gap: '8px', marginBottom: '32px' },
    backBtn: { background: 'none', border: 'none', color: '#64748b', fontSize: '13px', fontWeight: '700', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '8px', width: 'fit-content' },
    ...
    iconBox: (color) => ({ width: '36px', height: '36px', borderRadius: '10px', backgroundColor: `${color}10`, display: 'flex', alignItems: 'center', justifyContent: 'center', color: color, flexShrink: 0 }),
    sectionHeading: { fontSize: '17px', fontWeight: '800', margin: '0' },
    
    dropZone: (active) => ({
      width: '100%', minHeight: '160px', borderRadius: '20px',
      border: active ? '3px solid #0ea5e9' : '2px dashed #e2e8f0',
      backgroundColor: active ? '#f0f9ff' : '#f8fafc',
      display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center',
      position: 'relative', cursor: 'pointer', transition: '0.3s', boxSizing: 'border-box'
    }),
    ...
    timePickerContainer: { display: 'flex', gap: '6px', alignItems: 'center', flexWrap: 'nowrap' },
    timeInput: { width: '42px', textAlign: 'center', padding: '10px 4px', borderRadius: '12px', border: '2px solid #f1f5f9', fontSize: '15px', fontWeight: '800', backgroundColor: '#f8fafc', outline: 'none' },
    periodWrapper: { display: 'flex', gap: '4px', backgroundColor: '#f1f5f9', padding: '4px', borderRadius: '14px' },
    periodToggle: (active) => ({ padding: '8px 10px', borderRadius: '10px', border: 'none', backgroundColor: active ? '#000' : 'transparent', color: active ? '#fff' : '#64748b', fontWeight: '800', cursor: 'pointer', fontSize: '11px', transition: '0.2s' }),
    ...
    input: { width: '100%', backgroundColor: '#f8fafc', border: '2px solid #f1f5f9', borderRadius: '14px', padding: '12px', fontSize: '15px', fontWeight: '600', outline: 'none', boxSizing: 'border-box' },
    textarea: { width: '100%', backgroundColor: '#f8fafc', border: '2px solid #f1f5f9', borderRadius: '14px', padding: '12px', fontSize: '15px', fontWeight: '600', outline: 'none', minHeight: '100px', boxSizing: 'border-box' },
    
    tierCard: { backgroundColor: '#f8fafc', borderRadius: '20px', padding: '16px', border: '1px solid #f1f5f9', marginBottom: '12px' },
    tierInput: { background: 'none', border: 'none', borderBottom: '2px solid #e2e8f0', fontSize: '15px', fontWeight: '900', padding: '4px 0', outline: 'none', width: '100%', marginBottom: '12px' },
    
    publishCard: { backgroundColor: '#0f172a', borderRadius: '24px', padding: '24px', color: '#fff' },
    submitBtn: { width: '100%', backgroundColor: '#fff', color: '#0f172a', border: 'none', padding: '16px', borderRadius: '16px', fontSize: '15px', fontWeight: '900', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px', cursor: 'pointer' }
  };
        ...
        <div style={styles.errorBanner}>
          ...
          {!organizerSubaccount && (
            <button 
              ...
              style={{ marginLeft: 'auto', background: '#991b1b', color: '#fff', border: 'none', padding: '6px 10px', borderRadius: '8px', cursor: 'pointer', fontSize: '11px', whiteSpace: 'nowrap' }}
            >
              Link Now
            </button>
          )}
      ...
      <form onSubmit={handleSubmit} className="create-form-layout" style={{display:'flex', flexDirection:'column'}}>
        <div className="main-column">
          ...
          <div style={styles.formSection}>
            ...
            >
              {uploading ? (
                <div style={{textAlign: 'center'}}><Loader2 className="animate-spin" size={24} /> <p style={{fontSize:'12px'}}>Uploading... {uploadProgress}%</p></div>
              ) : (
                <div style={{textAlign: 'center'}}>
                  <Upload size={24} color="#94a3b8" style={{marginBottom: '8px'}}/>
                  <p style={{fontWeight: 800, margin: 0, fontSize: '13px'}}>Upload Images</p>
                </div>
              )}
          ...
          <div style={styles.formSection}>
            ...
            <div style={styles.inputGroup}>
              ...
              <div style={{position:'relative'}}>
                ...
                <input style={{...styles.input, paddingLeft:'38px'}} placeholder="Venue Name" value={eventData.location} onChange={(e) => setEventData({...eventData, location: e.target.value})} />
              </div>
            </div>
        ...
        <div className="publish-column">
          ...
          <div style={styles.formSection}>
            ...
            {tiers.map((tier) => (
              <div key={tier.id} style={styles.tierCard}>
                <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', gap:'10px'}}>
                   <input style={styles.tierInput} placeholder="e.g. VIP Table" value={tier.name} onChange={(e) => updateTier(tier.id, 'name', e.target.value)} />
                   <button type="button" onClick={() => removeTier(tier.id)} style={{background:'none', border:'none', color:'#fca5a5', padding:'0 0 12px 0'}}><Trash2 size={16}/></button>
                </div>
            ...
            <button type="button" onClick={addTier} style={{width:'100%', padding:'14px', borderRadius:'14px', border:'2px dashed #e2e8f0', background:'none', color:'#64748b', fontWeight:'800', fontSize:'13px', display:'flex', alignItems:'center', justifyContent:'center', gap:'8px'}}><Plus size={14}/> ADD TIER</button>
          </div>
          ...
          <div style={styles.publishCard}>
             <div style={{display:'flex', gap:'16px', marginBottom:'20px'}}>
                ...
                <div>
                   <h3 style={{margin:0, fontWeight:900, fontSize: '16px'}}>Publish Experience</h3>
                   <p style={{margin:0, color:'#94a3b8', fontSize:'12px'}}>Review your tiers before launching.</p>
                </div>
             </div>
             ...
             <div style={{display:'flex', justifyContent:'center', marginTop:'14px', color:'#4ade80', fontSize:'10px', fontWeight:800, gap:'5px'}}>
